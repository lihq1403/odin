 ...
 */
class Anthropic extends AbstractApi
{
    /**
     * @var Client[]
     */
    ...
    public function getClient(AnthropicConfig $config, ?ApiOptions $requestOptions = null, ?LoggerInterface $logger = null): Client
    {
        if (empty($config->getApiKey()) && ! $config->shouldSkipApiKeyValidation()) {
            throw new LLMInvalidApiKeyException('API key cannot be empty', null, 'Anthropic');
        }
